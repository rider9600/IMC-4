...
CODE_DIR = ROOT / "code"
OLD_FILE = ROOT / "submissions" / "54949" / "54949.py"
NEW_FILE = CODE_DIR / "strategy_csv_trend.py"
...
def run_replay(trader, prices_df):
    ...
    return {"pnl": cash + mtm, "positions": positions}


def main():
    df = pd.read_csv(ROOT / "data" / "prices_round_0_day_-1.csv", sep=";")

    old_trader = load_trader(OLD_FILE, "old_54949")
    new_trader = load_trader(NEW_FILE, "new_csvtrend")
    ...
    print(f"New csv-trend pnl: {new_res['pnl']:.2f}, pos={new_res['positions']}")
